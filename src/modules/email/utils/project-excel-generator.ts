...
export class ProjectExcelGenerator {
  ...
  private static createCombinedSheet(workbook: ExcelJS.Workbook, project: any, creator: any, company: any) {
    ...
    this.addRow(sheet, 'Submit Date', project.submitDate ? new Date(project.submitDate).toLocaleString() : 'N/A');
    ...
    if (project.client) {
      this.addSectionHeader(sheet, sheet.lastRow.number + 1, 'CLIENT INFORMATION');
      sheet.addRow([]);
      ...
      this.addRow(sheet, 'Communication Preference', project.client.communicationPreference ? 'Yes' : 'No');
    ...
    if (project.source) {
      this.addSectionHeader(sheet, sheet.lastRow.number + 1, 'SOURCE INFORMATION');
      sheet.addRow([]);
      ...
      this.addRow(sheet, 'Communication', project.source.communication ? 'Yes' : 'No');
    ...
    if (project.financial) {
      this.addSectionHeader(sheet, sheet.lastRow.number + 1, 'FINANCIAL INFORMATION');
      sheet.addRow([]);

      this.addRow(sheet, 'Subject Price', project.financial.subjectPrice || 'N/A');
      this.addRow(sheet, 'Concession', project.financial.concession || 'N/A');
      this.addRow(sheet, 'Extended Warranty', project.financial.extendedWarranty || 'N/A');
      this.addRow(sheet, 'Maintenance PMs', project.financial.maintenancePMs || 'N/A');
    ...
    if (project.transaction) {
      this.addSectionHeader(sheet, sheet.lastRow.number + 1, 'TRANSACTION INFORMATION');
      sheet.addRow([]);

      this.addRow(sheet, 'Current Meter', project.transaction.currentMeter || 'N/A');
      this.addRow(sheet, 'Meter Unit', project.transaction.meterUnit || 'N/A');
      this.addRow(sheet, 'Maintenance Records', project.transaction.maintenanceRecords || 'N/A');
      this.addRow(sheet, 'Inspection Report', project.transaction.inspectionReport || 'N/A');
      this.addRow(sheet, 'Structure', project.transaction.structure || 'N/A');
      this.addRow(sheet, 'Application', project.transaction.application || 'N/A');
      this.addRow(sheet, 'Environment', project.transaction.environment || 'N/A');
    ...
    if (project.utilizationScenarios && project.utilizationScenarios.length > 0) {
      this.addSectionHeader(sheet, sheet.lastRow.number + 1, 'UTILIZATION SCENARIOS');
      sheet.addRow([]);
  ...
  private static addSectionHeader(sheet: ExcelJS.Worksheet, rowNumber: number, title: string) {
  ...
  private static addUtilizationScenariosTable(sheet: ExcelJS.Worksheet, scenarios: any[]) {
    // Add header row
    const headerRow = sheet.addRow([
      'Scenario No',
      'Equipment ID',
